...
class Electrodomestico
{
	...
	protected:
		...
		float consumo_energetico(){
			if( getConsumo()== 'A')	return 	100;
			if( getConsumo()== 'B')	return 	80;
			if( getConsumo() =='C')return 	60;
			if( getConsumo() == 'D') return 50;
			if( getConsumo() == 'E')return 	30;
			if( getConsumo() == 'F')return 	10;
		}
		float CalcularPeso(){
...
class Lavadora : Electrodomestico{
	...
	void Mostrar_total(){
		...
		cout<<"\tSu carga es de = "<<getCarga()<<endl;
...
void llenarMan(Lavadora *arr[], int cant)
{
	...
	for(int i=0;i<cant;i++)
	{
		...
		arr[i] = new Lavadora(carga,precio, peso ,colores[i] , Tipo[i]);
	}
}
void llenar(Television *arr[], int cant)
{
	...
	for(int i=0;i<cant;i++)
	{
		...
		colores[j];
		Tipo[j];
		cout<<" 		COMPRA  # "<<i+1<<"Television"<<endl;
		cout<<"\t1.-Ingrese precio Base : ";cin>>precio;
		cout<<"\t2.-ingrese tipo de Color : "<<colores[i]<<endl;
		cout<<"\t3.-Ingrese tipo de Consumo energetico [ A Y F] "<<Tipo[i]<<endl;
