...
{	for (int j=0;j<=t-1;j++)
	{	v[j]=numReal(500,1000)+numReal(0,999)/100.0;  
...
int buscaBinaria(float v[],int bajo,int alto, float dato)
...
{	float vector[250],busca;
	...
	posicion=buscaBinaria(vector,0,tam-1,busca);
	cout<<endl;
	if (posicion==-1)
	{	cout<<"El dato "<<busca<<" no se encuentra en el vector";
	}
